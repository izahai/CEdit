# Vast AI: Mean-Norm Target-Global Pairwise Residual Evaluation

This workflow trains and evaluates `mean_norm_target_global_pairwise_residual_subspace`
...
`eval_target_global_pairwise_residual_subspace/train_config.yaml`, except for `anchor_mode`.
...
```bash
cd /workspace/CEdit
bash remote_scripts/eval_mean_norm_target_global_pairwise_residual_subspace/inspect_retain_low_rank.sh
```
...
```bash
ssh -p 45205 root@175.121.93.64 -L 8080:localhost:8080
cd /workspace/CEdit
nvidia-smi
...
bash remote_scripts/eval_mean_norm_target_global_pairwise_residual_subspace/run_all.sh
```
...
```bash
FORCE_RETRAIN=1 bash remote_scripts/eval_mean_norm_target_global_pairwise_residual_subspace/02_train.sh
FORCE_RESAMPLE=1 bash remote_scripts/eval_mean_norm_target_global_pairwise_residual_subspace/03_infer.sh
FORCE_EVAL=1 bash remote_scripts/eval_mean_norm_target_global_pairwise_residual_subspace/05_eval.sh
```
...
GCD_NUM_WORKERS=16 GCD_BATCH_SIZE=64 GCD_PREFETCH_FACTOR=2 \
  FORCE_EVAL=1 \
  bash remote_scripts/eval_mean_norm_target_global_pairwise_residual_subspace/05_eval.sh
```
...
```bash
ssh -p "${VAST_PORT}" "root@${VAST_HOST}" \
  'find /workspace/cedit_ce_eval_outputs_mean_norm_target_global_pairwise_residual_subspace/images -type f -name "*.png" 2>/dev/null | wc -l'
```
...
```text
/workspace/cedit_ce_eval_outputs_mean_norm_target_global_pairwise_residual_subspace/
...
scp -P "${VAST_PORT}" \
  "root@${VAST_HOST}:/workspace/cedit_ce_eval_outputs_mean_norm_target_global_pairwise_residual_subspace/gcd/summary.csv" \
  .
```
