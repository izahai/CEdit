...
load_workflow_config() {
    ...
    read -r -a CONFIG_NAMES <<< "${CONFIG_NAMES_RAW}"
}
...
train_config_for_name() {
    case "$1" in
        config_1|config_2) printf '%s/train_%s.yaml' "${WORKFLOW_DIR}" "$1" ;;
        *) printf 'Unknown training config: %s\n' "$1" >&2; return 1 ;;
    esac
}
...
with open(path, newline="", encoding="utf-8") as csv_file:
    for row in csv.DictReader(csv_file):
        if row.get("type") == "erase" and row.get("concept") not in targets:
            targets.append(row["concept"])
...
target_count_for_benchmark() {
    local benchmark_name=$1
    local target_count=${benchmark_name%_celebrity}
    [[ "${target_count}" =~ ^[0-9]+$ ]] || {
        printf 'Cannot infer target count from benchmark: %s\n' "${benchmark_name}" >&2
        return 1
    }
    printf '%s' "${target_count}"
}
