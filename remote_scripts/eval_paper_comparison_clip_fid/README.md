# Vast AI: Paper Comparison with MS-COCO CLIP/FID
...
```bash
FORCE_RESAMPLE=1 bash remote_scripts/eval_paper_comparison_clip_fid/02_generate_original.sh
FORCE_RETRAIN=1 bash remote_scripts/eval_paper_comparison_clip_fid/03_train.sh
FORCE_RESAMPLE=1 bash remote_scripts/eval_paper_comparison_clip_fid/04_infer_edits.sh
FORCE_EVAL=1 bash remote_scripts/eval_paper_comparison_clip_fid/06_eval.sh
FORCE_RESAMPLE=1 bash remote_scripts/eval_paper_comparison_clip_fid/07_generate_mscoco.sh
FORCE_EVAL=1 bash remote_scripts/eval_paper_comparison_clip_fid/08_eval_clip_fid.sh
```
...
1,000 original MS-COCO images, and six sets of 1,000 edited MS-COCO images.
...
├── mscoco/
│   ├── original/coco/original/*.png
│   └── <method>/<benchmark>/coco/edit/*.png
├── clip_fid/summary.csv
└── gcd/
...
The final summary repeats the checkpoint-level `mscoco_n_images`,
`mscoco_clip_score`, and `mscoco_fid_vs_original` values on its erase and
retain rows. CLIP score is multiplied by 100, matching `scripts/eval_multi.sh`;
lower FID means the edited distribution remains closer to original SD v1.4.
The setup pins `scipy<1.18` because `torch_fidelity==0.3.0` still calls the
`scipy.linalg.sqrtm(..., disp=False)` API removed in SciPy 1.18.
