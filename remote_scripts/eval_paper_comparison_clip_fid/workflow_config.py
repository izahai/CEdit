...
def build_environment(config, workflow_dir):
    ...
    values = {
        ...
        "METHODS_RAW": get_value(
            "METHODS_RAW", string_list(config, "experiment", "methods")
        ),
        ...
        "CLIP_BATCH_SIZE": get_value(
            "CLIP_BATCH_SIZE",
            required(config, "experiment", "clip_batch_size"),
        ),
        "CLIP_MODEL": get_value(
            "CLIP_MODEL", required(config, "experiment", "clip_model")
        ),
        "GCD_USE_CUDA": get_value(
            "GCD_USE_CUDA", required(config, "experiment", "gcd_use_cuda")
        ),
        "GCD_NUM_WORKERS": get_value(
            "GCD_NUM_WORKERS",
            required(config, "experiment", "gcd_num_workers"),
        ),
        "GCD_BATCH_SIZE": get_value(
            "GCD_BATCH_SIZE", required(config, "experiment", "gcd_batch_size")
        ),
        "GCD_PREFETCH_FACTOR": get_value(
            "GCD_PREFETCH_FACTOR",
    ...
    values["CLIP_FID_OUTPUT_DIR"] = get_value(
        "CLIP_FID_OUTPUT_DIR", os.path.join(values["OUTPUT_ROOT"], "clip_fid")
    )
    return values
