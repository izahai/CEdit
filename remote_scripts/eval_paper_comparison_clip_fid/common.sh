...
TMUX_LOG_FORMAT="raw-paper-comparison-v1"
...
mscoco_image_dir_for_run() {
    if [[ "$1" == "original" ]]; then
        ...
        printf '%s/%s/%s/coco/edit' "${MSCOCO_IMAGE_ROOT}" "$1" "$2"
    fi
}
...
train_config_for_method() {
    case "$1" in
        ...
        target_global_pairwise_residual_subspace)
            ...
            ;;
        *)
            printf 'Method does not have a training config: %s\n' "$1" >&2
            return 1
            ;;
    esac
}
