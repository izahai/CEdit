...
IMAGE_DIR="${IMAGE_ROOT}/${ANCHOR_MODE}/${BENCHMARK_NAME}/retain/edit"
CSV_PATH="${GCD_OUTPUT_DIR}/${ANCHOR_MODE}_retain.csv"
XLSX_PATH="${GCD_OUTPUT_DIR}/${ANCHOR_MODE}_retain.xlsx"
LOG_PATH="${GCD_OUTPUT_DIR}/${ANCHOR_MODE}_retain.log"
require_directory "${IMAGE_DIR}"

if [[ -f "${CSV_PATH}" && "${FORCE_EVAL:-0}" != "1" ]]; then
    printf 'Retain evaluation exists; skipping: %s\n' "${CSV_PATH}"
    exit 0
fi
...
printf 'Retain evaluation complete: %s\n' "${CSV_PATH}"
