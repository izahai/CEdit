...
GPU_ID="${GPU_ID:-0}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/logs/eval_100_style}"
...
for ((i=start; i<${#stages[@]}; i++)); do
    ...
    if [[ "${stage}" == setup ]]; then
        ...
        if command -v uv >/dev/null 2>&1; then
            uv pip install --python "${PYTHON_BIN}" -r "${REPO_ROOT}/requirements.txt" 'scipy<1.18'
        else
            "${PYTHON_BIN}" -m pip install -r "${REPO_ROOT}/requirements.txt" 'scipy<1.18'
        fi
        CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" -c 'import torch, diffusers, transformers, torch_fidelity; assert torch.cuda.is_available()'
    else
