...
for config_name in "${CONFIG_NAMES[@]}"; do
    for benchmark_name in "${BENCHMARK_NAMES[@]}"; do
        checkpoint_path="$(checkpoint_dir_for_run "${config_name}" "${benchmark_name}")/weight.pt"
        ...
        if [[ "${erase_complete}" == "1" && "${retain_complete}" == "1" && "${FORCE_RESAMPLE:-0}" != "1" ]]; then
            printf '%s/%s erase and retain images are already complete.\n' \
        ...
        for split in erase retain; do
            ...
            [[ "${image_count}" == "${EXPECTED_IMAGES_PER_SPLIT}" ]] || {
                printf '%s/%s expected %s %s images in %s, found %s\n' \
                    "${config_name}" "${benchmark_name}" \
                    "${EXPECTED_IMAGES_PER_SPLIT}" "${split}" "${image_dir}" \
                    "${image_count}" >&2
                exit 1
            }
            printf '%s/%s %s inference complete: %s images\n' \
                "${config_name}" "${benchmark_name}" "${split}" "${image_count}"
        done
    done
done
