...
enable_tmux_logging() {
    ...
    if [[ "${pane_log_format}" == "${TMUX_LOG_FORMAT}" ]]; then
        printf 'clean tmux pane logging is already active: %s\n' \
            "${TMUX_LOG_PATH}"
        return 0
    fi

    printf -v log_command '%q %q --append %q' \
        "${BOOTSTRAP_PYTHON}" \
        "${WORKFLOW_DIR}/clean_tmux_log.py" \
        "${TMUX_LOG_PATH}"
    tmux pipe-pane -t "${pane_id}" "${log_command}"
    tmux set-option -p -t "${pane_id}" \
        @cedit_tmux_log_format "${TMUX_LOG_FORMAT}"
    printf 'clean tmux pane logging enabled: %s\n' "${TMUX_LOG_PATH}"
}
