...
def load_training_configs(workflow_dir):
    paths = {
        ...
        METHOD_CFB: Path(workflow_dir) / "train_config_cfb.yaml",
    }
    configs = {}
...
def training_metadata(method, task, train_configs):
    ...
    return {
        ...
        "erase_style": config.get("erase_style", False),
        "retain_projection_rank": config.get("retain_projection_rank", ""),
        "threshold": config["threshold"],
        "retain_scale": config["retain_scale"],
        "residual_scale": config.get("residual_scale", ""),
        "use_k2": config.get("use_k2", ""),
        "anchor_steps": config.get("anchor_steps", ""),
        "anchor_lr": config.get("anchor_lr", ""),
    ...
    for task in tasks:
        for method in methods:
            ...
            summaries.append({
                ...
                "erase_style": first["erase_style"],
                "retain_projection_rank": first["retain_projection_rank"],
                "threshold": first["threshold"],
                "retain_scale": first["retain_scale"],
                "residual_scale": first["residual_scale"],
                "use_k2": first["use_k2"],
                "anchor_steps": first["anchor_steps"],
                "anchor_lr": first["anchor_lr"],
                "anchor_batch_size": first["anchor_batch_size"],
                "validation_samples": first["validation_samples"],
                "validation_interval": first["validation_interval"],
...
def build_comparison_rows(summary_rows, tasks):
    ...
    for task in tasks:
        ...
        comparisons.append({
            ...
            "cfb_aug_num": cfb["aug_num"],
            "cfb_retain_projection_rank": cfb["retain_projection_rank"],
            "cfb_anchor_steps": cfb["anchor_steps"],
            "original_target_clip_score": original["target_clip_score_mean"],
            "legacy_target_clip_score": legacy["target_clip_score_mean"],
            "cfb_target_clip_score": cfb["target_clip_score_mean"],
            "target_clip_score_improvement": (
                legacy["target_clip_score_mean"]
                - cfb["target_clip_score_mean"]
            ),
            "legacy_non_target_fid": legacy["non_target_fid_mean"],
            "cfb_non_target_fid": cfb["non_target_fid_mean"],
            "non_target_fid_improvement": (
                legacy["non_target_fid_mean"] - cfb["non_target_fid_mean"]
            ),
            "original_mscoco_clip_score": original["mscoco_clip_score"],
            "legacy_mscoco_clip_score": legacy["mscoco_clip_score"],
            "cfb_mscoco_clip_score": cfb["mscoco_clip_score"],
            "mscoco_clip_score_improvement": (
                cfb["mscoco_clip_score"] - legacy["mscoco_clip_score"]
            ),
            "legacy_mscoco_fid": legacy["mscoco_fid_vs_original"],
            "cfb_mscoco_fid": cfb["mscoco_fid_vs_original"],
            "mscoco_fid_improvement": (
                legacy["mscoco_fid_vs_original"]
                - cfb["mscoco_fid_vs_original"]
            ),
        })
    return comparisons


def runtime_fingerprint(config, args, train_configs):
    values = {
...
def evaluate(args):
    ...
    run_hash = runtime_fingerprint(config, args, train_configs)
