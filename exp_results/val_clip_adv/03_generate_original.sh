...
for raw_content in "${content_items[@]}"; do
    content="${raw_content# }"
    content="${content% }"
    image_dir="$(few_image_dir original "${content}")"
...
for raw_content in "${content_items[@]}"; do
    content="${raw_content# }"
    content="${content% }"
    assert_png_count "$(few_image_dir original "${content}")" \
        "${expected_per_content}" "original/${content}"
done
printf 'Original style generation complete.\n'
