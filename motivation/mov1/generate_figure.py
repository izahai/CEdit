#!/usr/bin/env python3
"""Build the three-panel real-CLIP common-anchor motivation figure."""
...
def write_projection_csv(path, names, coordinates, tgprs_endpoints):
    with path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        for index, name in enumerate(names):
            writer.writerow({
                ...
                "tgprs_end_pc1": tgprs_endpoints[index, 0].item(),
                "tgprs_end_pc2": tgprs_endpoints[index, 1].item(),
            })
        writer.writerow({
            "prompt": "person",
            "kind": "anchor",
...
def render_figure(
    ...
    tgprs_axis = axes[1]
    target_xy = coordinates[:-1].numpy()
    endpoint_xy = tgprs_endpoints.numpy()
    displacement_xy = endpoint_xy - target_xy
    # Translate each target-anchor pair to its own grid cell.  This preserves the
    # projected residual direction (and relative length, up to clipping) without
    # creating a false common center from crossings in absolute PCA coordinates.
    ...
    all_x = list(target_xy[:, 0]) + [anchor_xy[0]] + list(endpoint_xy[:, 0])
    all_y = list(target_xy[:, 1]) + [anchor_xy[1]] + list(endpoint_xy[:, 1])
    ...
    render_native_panel_pdfs(
        ...
        explained=[value.item() for value in explained],
        spectra={
            method: summaries[method]["relative_singular_values"]
            for method in ("legacy", "tgprs")
        },
        ranks={
            method: summaries[method]["numerical_rank"]
            for method in ("legacy", "tgprs")
        },
        rank_tolerance=summaries["legacy"]["relative_rank_tolerance"],
        rank=rank,
...
def run(config, output_dir):
    ...
    if anchor_prompt != "person":
        raise ValueError('MOV3 fixes the shared anchor prompt to exactly "person"')
    ...
    coordinates, explained, components = pca_projection(targets, anchor)
    tgprs_projection = tgprs.float().cpu() @ components.T
    tgprs_endpoints = coordinates[:-1] + tgprs_projection
    ...
    print(f"Saved MOV3 outputs to {output_dir}")
