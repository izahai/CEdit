...
mode) sẽ log index và prompt target cung cấp shared residual. `legacy` và
`shared_residual_mean` không in source log này.
...
CLI sử dụng `--residual_scale`, mặc định là `1.0` và chấp nhận mọi số hữu hạn. Giá
trị âm đảo hướng residual, còn `0` tạo edit statistic bằng không. Residual được scale
