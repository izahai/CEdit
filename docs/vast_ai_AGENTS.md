# Agent guide for this instance

This single file IS the complete agent guide — the full text of all 2 guide(s)
is concatenated below (the same files also live individually under /etc/vast_agents/, but you
do not need to open them; everything is here). Read this whole file before acting
on this instance. The sections are cumulative:

  1. base.md — Managing this instance (AI agent guide)
  2. pytorch.md — PyTorch (this image)

Keep reading past the base section: the per-image sections below document services
and APIs (endpoints, wrappers, helpers) you would otherwise miss — read them before
calling an API, exposing a service, or setting up a model.
